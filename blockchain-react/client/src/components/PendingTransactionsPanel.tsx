...
const PendingTransactionsPanel: React.FC<PendingTransactionsPanelProps> = ({
  ...
  return (
    <>
      ...
      <pre className="pending-transactions__list">{formattedTransactions || 'No pending transactions yet.'}</pre>
      <div className="pending-transactions__form">
        <button disabled={disabled} onClick={() => onGenerateBlock()} className="ripple" type="button">
          GENERATE BLOCK
        </button>
      </div>
